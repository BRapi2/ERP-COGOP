...
import { DepartamentoService } from "../../../departamento/application/services/Departamento.service";

@injectable()
export class ProvinciaController extends BaseController {
    constructor(
        @inject(ProvinciaService) private service: ProvinciaService,
        @inject(DepartamentoService) private serviceDepartamento: DepartamentoService
    ) { super(); }
    ...
    public async list(query: ListQueryDto): Promise<HttpResponseInit> {
        try {
            ...
            return this.handleSuccess({
                ...
                departamento: await this.serviceDepartamento.findAll()
            });
        } catch (error) {
    ...
    public async save(req: HttpRequest, id): Promise<HttpResponseInit> {
        try {
            const dto = req.validatedBody as ProvinciaDto;
            const result = dto.id ? await this.service.update(dto.id, dto) : await this.service.create(dto);
            await LogService.logSuccess(!dto.id ? "CREAR_PROVINCIA" : "ACTUALIZAR_PROVINCIA", "Operación exitosa", result, id);
