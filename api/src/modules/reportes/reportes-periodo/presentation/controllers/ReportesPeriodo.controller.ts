...
import { ReportesPeriodoService } from "../../application/services/ReportesPeriodo.service";
...
@injectable()
export class ReportesPeriodoController extends BaseController {
    constructor(
        @inject(ReportesPeriodoService) private service: ReportesPeriodoService
    ) { super(); }
    ...
    public async save(req: HttpRequest, id): Promise<HttpResponseInit> {
        try {
            const dto = req.validatedBody as ReportesPeriodoDto;
            const result = dto.id
                ? await this.service.update(dto.id, dto)
                : await this.service.create(dto);

            await LogService.logSuccess(
                !dto.id ? "CREAR_REPORTE_PERIODO" : "ACTUALIZAR_REPORTE_PERIODO",
                "Operación exitosa",
                result,
                id
            );
