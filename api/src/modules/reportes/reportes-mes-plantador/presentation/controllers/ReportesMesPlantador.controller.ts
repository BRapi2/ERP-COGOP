...
import { ReportesMesPlantadorService } from "../../application/services/ReportesMesPlantador.service";
...
@injectable()
export class ReportesMesPlantadorController extends BaseController {
    constructor(
        @inject(ReportesMesPlantadorService) private service: ReportesMesPlantadorService,
    ) { super(); }
    ...
    public async list(query: ListQueryDto, userId: number): Promise<HttpResponseInit> {
        try {
            ...
            const defaultSort: FindOptionsOrder<ReportesMesPlantadorModel> = { id: "ASC" };
            const order = parseSort<ReportesMesPlantadorModel>(sort, ["id"], defaultSort);

            const findOptions: FindManyOptions<ReportesMesPlantadorModel> = { order };
    ...
    public async listById(id: number): Promise<HttpResponseInit> {
        try {
            ...
            if (!reporte) {
                return this.handleError(new Error(`Reporte Mes Plantador con id ${id} no encontrado.`), 404);
            }
    ...
    public async save(req: HttpRequest, userId: number): Promise<HttpResponseInit> {
        try {
            const dto = req.validatedBody as ReportesMesPlantadorDto;
            const result = dto.id
                ? await this.service.update(dto.id, dto)
                : await this.service.create(dto);

            await LogService.logSuccess(
                !dto.id ? "CREAR_REPORTE_MES_PLANTADOR" : "ACTUALIZAR_REPORTE_MES_PLANTADOR",
                "Operación exitosa",
                result,
                userId
            );
