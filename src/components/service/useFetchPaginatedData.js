...
const useFetchPaginatedData = (endpointPath, paramsConfig = {}) => {
  const [data, setData] = useState(null);
  const [dataTotal, setDataTotal] = useState(null);
  const [dataPaginacion, setDataPaginacion] = useState(null);
  const [roles, setRoles] = useState([]); // <- NUEVO
  ...
  const fetchData = useCallback(async (signal) => {
    ...
    try {
      ...
      for (const key in otrosFiltros) {
        if (Object.hasOwnProperty.call(otrosFiltros, key) &&
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Error ${response.status}: ${response.statusText || 'Error del servidor'}`);
      }

      const responseData = await response.json();
      // ---- AQUI VIENE EL CAMBIO CLAVE ----
      let rolesArr = [];
      if (responseData.data && (responseData.status === 'SUCCESS' || responseData.status === 'success')) {
        if (responseData.data.lista) {
          ...
          if (Array.isArray(responseData.data.lista.roles)) {
            rolesArr = responseData.data.lista.roles;
          }
        } else if (responseData.data.content) {
          setData(responseData.data.content);
          setDataPaginacion(responseData.data);
          if (Array.isArray(responseData.data.roles)) {
            rolesArr = responseData.data.roles;
          }
      ...
      if (err.name !== 'AbortError') {
        console.error(`Error en useFetchPaginatedData (${endpointPath}):`, err);
  ...
  }, [endpointPath, stringifiedParamsConfig]);
